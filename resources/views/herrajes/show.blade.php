        ...
        <div class="p-6">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div class="lg:col-span-8">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Herraje *</label>
                    <select x-model="form.descripcion"
                            class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-800 text-base"
                            required>
                        <option value="" disabled selected>Seleccionar herraje...</option>
            ...
            <div class="mt-6 flex justify-end">
                <button @click="agregarItem()"
                        :disabled="agregando || !form.descripcion"
                        class="inline-flex items-center gap-3 px-8 py-3.5 bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white font-bold rounded-lg shadow-lg hover:shadow-xl transition-all transform hover:scale-105 disabled:cursor-not-allowed disabled:transform-none">
        ...
        return {
            ...
            async agregarItem() {
                if (!this.form.descripcion) {
                    window.showAlert?.('warning', 'Selecciona un herraje');
                    return;
                }
                ...
                try {
                    ...
                    window.showAlert?.('error', 'Error al agregar');
                } finally {
            ...
            async updateItem(it) {
                try {
                    ...
                    if (data.success) {
                        await this.cargarItems();
                        window.showAlert?.('success', 'Actualizado');
                    }
                } catch (e) {
                    window.showAlert?.('error', 'Error al actualizar');
                }
            },
            ...
            async eliminarItem(it) {
                ...
                try {
                    ...
                    if (data.success) {
                        await this.cargarItems();
                        window.showAlert?.('success', 'Eliminado');
                    }
                } catch (e) {
                    window.showAlert?.('error', 'Error al eliminar');
                }
            }
        }
    }
